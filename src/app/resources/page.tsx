...
function TabButton({ label, active, onClick }: { label: string; active: boolean; onClick: () => void }) {
  return (
    <button onClick={onClick} className={`shrink-0 px-5 py-2.5 rounded-full text-sm font-semibold transition-all duration-300 ${active ? "gold-gloss shadow-glow" : "glass text-graphite-600 hover:-translate-y-0.5"}`}>
      {label}
    </button>
  );
}
...
function IconPanel({ name }: { name: string }) {
  ...
  return (
    <div className="h-36 w-full flex items-center justify-center" style={{ background: "linear-gradient(135deg, #6EA8B6 0%, #3F6F7C 100%)" }}>
      ...
      <div className="max-w-5xl mx-auto px-6 pb-24">
        {tab === "Overview" && (
          <div className="space-y-6">
            <div className="relative rounded-3xl overflow-hidden flex items-end" style={{ height: "320px", background: "linear-gradient(135deg, #3F6F7C 0%, #1F3A41 100%)" }}>
              <div className="absolute inset-0 opacity-20 flex items-center justify-center">
                <Icons.HeartPulse size={220} className="text-gold-200" strokeWidth={1} />
              </div>
